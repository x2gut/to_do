...
function ActiveTasks({ tasksData, setTasksData, activeButton }) {
  ...
  const dividedArray = divideArrayIntoChunks(tasksData.active, 10);
  ...
  const handleShowMoreClick = () => {
    setCurrentLevel((prevLevel) => Math.min(prevLevel + 1, dividedArray.length));
  };

  return (
    <div className={`active-tasks ${activeButton === "active" ? "active" : ""}`}>
            ...
            {chunk.map((task) => (
              <li key={task.id}>
                ...
                <div className="manipulate-btn">
                  ...
                  <button className="delete-task" onClick={() => handleTaskDelete(task.id)}>
                    <AiOutlineDelete size={21} />
                  </button>
                  <button className="complete-task" onClick={() => handleTaskComplete(task.id)}>
                    <svg
                      width="18"
                      height="13"
                      viewBox="0 0 18 13"
